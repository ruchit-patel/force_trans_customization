...
function testStatistics() {
  ...
  const openTickets = mockIssues.filter(issue => issue.status === 'Open').length
  const assignedToMe = mockIssues.filter(issue => issue.raised_by === mockUser).length
  const actionableTickets = mockIssues.filter(issue => ['Open', 'Replied'].includes(issue.status)).length
  const responseTickets = mockIssues.filter(issue => issue.status === 'Replied').length
