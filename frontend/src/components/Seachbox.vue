    ...
    <div class="relative w-full max-w-2xl mx-auto">
        ...
        <div class="relative group">
            ...
            <input 
                ...
                placeholder="Search issues by title, description, ID..."
                class="w-full h-14 pl-12 pr-20 text-base bg-white border border-slate-200 rounded-2xl 
        ...
        <div v-if="showSuggestions" 
             class="absolute z-50 w-full mt-3 bg-white border border-slate-200 rounded-2xl shadow-2xl 
            ...
            <div class="max-h-80 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
                <div v-if="searchSuggestions.length === 0 && searchQuery.length >= 2" 
                ...
                <div v-for="(suggestion, index) in searchSuggestions" 
                     ...
                     class="group px-4 py-4 hover:bg-blue-50/50 cursor-pointer transition-all duration-150 
                            ...
                            <div class="flex items-start justify-between">
                                <div class="flex-1 min-w-0">
                                    <h4 class="text-sm font-semibold text-slate-900 truncate group-hover:text-blue-900 
                                        ...
                                        <span v-html="highlightMatch(suggestion.subject || suggestion.name, searchQuery)"></span>
                                    </h4>
                ...
                <div class="flex items-center justify-between text-xs text-slate-500">
                    <div class="flex items-center space-x-4">
                        <span class="flex items-center">
                            <kbd class="px-1.5 py-0.5 bg-white border border-slate-200 rounded text-xs font-mono mr-1">↵</kbd>
                            to select
                        </span>
                        <span class="flex items-center">
                            <kbd class="px-1.5 py-0.5 bg-white border border-slate-200 rounded text-xs font-mono mr-1">↑↓</kbd>
                    ...
                    <span class="flex items-center">
                        <kbd class="px-1.5 py-0.5 bg-white border border-slate-200 rounded text-xs font-mono mr-1">esc</kbd>
                        to close
                    </span>
                </div>
            </div>
        ...
        <!-- Backdrop -->
        <div v-if="showSuggestions" 
             class="fixed inset-0 z-40 bg-black/5 backdrop-blur-sm" 
             @click="hideSuggestions"></div>
    </div>
</template>
...
const searchSuggestions = computed(() => {
    ...
    const results = props.issues
        .filter(issue =>
            ...
            (issue.name && issue.name.toLowerCase().includes(query)) ||
            (issue.customer && issue.customer.toLowerCase().includes(query)) ||
            (issue.raised_by && issue.raised_by.toLowerCase().includes(query))
        )
...
const handleFocus = () => {
    ...
    if (searchQuery.value.length >= 2 && searchSuggestions.value.length > 0) {
        showSuggestions.value = true
    }
}
