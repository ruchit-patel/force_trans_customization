...
console.assert(result.length === 3, 'Should return all issues when no filters applied')
...
console.assert(result.every(issue => issue.status === 'Open'), 'All results should have Open status')
...
console.assert(result[0].priority === 'High', 'Should return high priority issue')
...
console.assert(result[0].name === 'ISS-001', 'Should return correct issue with multiple filters')
...
console.assert(assigneeOptions.length === 3, 'Should return 3 assignee options (All + 2 unique)')
console.assert(assigneeOptions[0].label === 'All Assignees', 'First option should be "All Assignees"')
...
filterComposable.filters.value.status = 'Open'
filterComposable.resetFilters()
console.assert(filterComposable.searchQuery.value === '', 'Search query should be reset')
console.assert(filterComposable.filters.value.status === '', 'Status filter should be reset')

// Test hasActiveFilters computed property
console.log('Testing hasActiveFilters...')
console.assert(filterComposable.hasActiveFilters.value === false, 'Should have no active filters initially')
filterComposable.searchQuery.value = 'test'
console.assert(filterComposable.hasActiveFilters.value === true, 'Should detect active search filter')
